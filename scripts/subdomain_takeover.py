#!/usr/bin/env python3
# Subdomain Takeover Script for SWAPTT
import sys
import time
import random
import re
...
def run_subdomain_takeover(target):
    print(f"Running Subdomain Takeover on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT Subdomain Takeover")
    ...
    elif "Subdomain Takeover" == "IP Lookup":
    ...
    elif "Subdomain Takeover" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "Subdomain Takeover" == "Headers Scanner":
    ...
    elif "Subdomain Takeover" == "SSL/TLS Checker":
    ...
    elif "Subdomain Takeover" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "Subdomain Takeover" == "WAF Detector":
    ...
    elif "Subdomain Takeover" == "DNS Resolver" or "Subdomain Takeover" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "Subdomain Takeover" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        target = "example.com"
    result = run_subdomain_takeover(target)
    print(result)
