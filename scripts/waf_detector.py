#!/usr/bin/env python3
# WAF Detector Script for SWAPTT
import sys
import time
import random
import re
...
def run_waf_detector(target):
    print(f"Running WAF Detector on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT WAF Detector")
    ...
    elif "WAF Detector" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "WAF Detector" == "Headers Scanner":
    ...
    elif "WAF Detector" == "SSL/TLS Checker":
    ...
    elif "WAF Detector" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "WAF Detector" == "WAF Detector":
    ...
    elif "WAF Detector" == "DNS Resolver" or "WAF Detector" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "WAF Detector" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
