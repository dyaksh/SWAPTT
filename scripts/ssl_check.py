#!/usr/bin/env python3
# SSL/TLS Checker Script for SWAPTT
import sys
import time
import random
import re
...
def run_ssl_check(target):
    print(f"Running SSL/TLS Checker on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT SSL/TLS Checker")
    ...
    elif "SSL/TLS Checker" == "IP Lookup":
    ...
    elif "SSL/TLS Checker" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "SSL/TLS Checker" == "Headers Scanner":
    ...
    elif "SSL/TLS Checker" == "SSL/TLS Checker":
    ...
    elif "SSL/TLS Checker" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "SSL/TLS Checker" == "WAF Detector":
    ...
    elif "SSL/TLS Checker" == "DNS Resolver" or "SSL/TLS Checker" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "SSL/TLS Checker" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
