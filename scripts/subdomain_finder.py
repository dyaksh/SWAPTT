#!/usr/bin/env python3
# Subdomain Finder Script for SWAPTT
import sys
import time
import random
import re
...
def run_subdomain_finder(target):
    print(f"Running Subdomain Finder on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT Subdomain Finder")
    ...
    elif "Subdomain Finder" == "IP Lookup":
    ...
    elif "Subdomain Finder" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "Subdomain Finder" == "Headers Scanner":
    ...
    elif "Subdomain Finder" == "SSL/TLS Checker":
    ...
    elif "Subdomain Finder" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "Subdomain Finder" == "WAF Detector":
    ...
    elif "Subdomain Finder" == "DNS Resolver" or "Subdomain Finder" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "Subdomain Finder" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        target = "example.com"
    result = run_subdomain_finder(target)
    print(result)
