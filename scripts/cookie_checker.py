#!/usr/bin/env python3
# Cookie Checker Script for SWAPTT
import sys
import time
import random
import re
...
def run_cookie_checker(target):
    print(f"Running Cookie Checker on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT Cookie Checker")
    ...
    elif "Cookie Checker" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "Cookie Checker" == "Headers Scanner":
    ...
    elif "Cookie Checker" == "SSL/TLS Checker":
    ...
    elif "Cookie Checker" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "Cookie Checker" == "WAF Detector":
    ...
    elif "Cookie Checker" == "DNS Resolver" or "Cookie Checker" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "Cookie Checker" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        target = "example.com"
    result = run_cookie_checker(target)
    print(result)
