#!/usr/bin/env python3
# Website Recon Script for SWAPTT
import sys
import time
import random
import re
...
def run_website_recon(target):
    print(f"Running Website Recon on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT Website Recon")
    ...
    elif "Website Recon" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "Website Recon" == "Headers Scanner":
    ...
    elif "Website Recon" == "SSL/TLS Checker":
    ...
    elif "Website Recon" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "Website Recon" == "WAF Detector":
    ...
    elif "Website Recon" == "DNS Resolver" or "Website Recon" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "Website Recon" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        target = "example.com"
    result = run_website_recon(target)
    print(result)
