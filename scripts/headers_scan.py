#!/usr/bin/env python3
# Headers Scanner Script for SWAPTT
import sys
import time
import random
import re
...
def run_headers_scan(target):
    print(f"Running Headers Scanner on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT Headers Scanner")
    ...
    elif "Headers Scanner" == "IP Lookup":
    ...
    elif "Headers Scanner" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "Headers Scanner" == "Headers Scanner":
    ...
    elif "Headers Scanner" == "SSL/TLS Checker":
    ...
    elif "Headers Scanner" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "Headers Scanner" == "WAF Detector":
    ...
    elif "Headers Scanner" == "DNS Resolver" or "Headers Scanner" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "Headers Scanner" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        target = "example.com"
    result = run_headers_scan(target)
    print(result)
