#!/usr/bin/env python3
# Command Injection Script for SWAPTT
import sys
import time
import random
import re
...
def run_command_injection(target):
    print(f"Running Command Injection on {target}")
    print("Initializing...")
    time.sleep(1)
    ...
    output = []
    output.append(f"SWAPTT Command Injection")
    ...
    elif "Command Injection" == "IP Lookup":
    ...
    elif "Command Injection" in ["Port Scanner Nmap", "Nmap", "Full Scan", "Small Scan"]:
    ...
    elif "Command Injection" == "Headers Scanner":
    ...
    elif "Command Injection" == "SSL/TLS Checker":
    ...
    elif "Command Injection" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
    ...
    elif "Command Injection" == "WAF Detector":
    ...
    elif "Command Injection" == "DNS Resolver" or "Command Injection" == "DNS Enumeration":
    ...
    output.append("-" * 50)
    if "Command Injection" in ["XSS Exploiter", "SQLi Exploiter", "LFI/RFI Tester", "Command Injection"]:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        target = "example.com"
    result = run_command_injection(target)
    print(result)
